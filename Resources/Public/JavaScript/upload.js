  ...
  $(function() {
    ...
    $resource.change(function(e) {
      ...
      if (filesize > maximumFileUploadSize) {
        ...
        if (window.Typo3Neos) {
          message = window.Typo3Neos.I18n.translate('error.fileSizeExceedsAllowedLimit', message, 'Psmb.Newsletter', 'Modules', [readableFileSize, readableMaximumFileSize]);
        ...
        $(this.form).on('submit.invalidfile', function(e) {
          ...
          if (window.Typo3Neos) {
            message = window.Typo3Neos.I18n.translate('error.cannotUploadFile', message, 'Psmb.Newsletter', 'Modules');
      ...
      if (type !== 'text/csv') {
        if (window.Typo3Neos) {
          message = window.Typo3Neos.I18n.translate('error.fileType', message, 'Psmb.Newsletter', 'Modules', [readableFileSize, readableMaximumFileSize]);
          window.Typo3Neos.Notification.error(message);
        } else {
          alert(message);
        }
    ...
    if (isSafari) {
      $resource.closest('form').on('submit.safari', function(e) {
        if (!$resource.val()) {
          ...
          if (window.Typo3Neos) {
            message = window.Typo3Neos.I18n.translate('error.noFileSelected', message, 'Psmb.Newsletter', 'Modules');
            window.Typo3Neos.Notification.warning(message);
          } else {
            alert(message);
          }
